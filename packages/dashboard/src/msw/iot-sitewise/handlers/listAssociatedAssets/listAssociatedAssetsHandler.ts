...
export function listAssociatedAssetsHandler() {
  return rest.get<ListAssociatedAssetsRequest>(LIST_ASSOCIATED_ASSETS_URL, (req, res, ctx) => {
    ...
    if (traversalDirection === 'CHILD') {
      const childAssets = ASSET_HIERARCHY.findChildren(assetId, hierarchyId ?? '') ?? [];
      const childAssetSummaries = childAssets.map(summarizeAssetDescription);
      ...
      const parentAssetSummary = parentAsset ? summarizeAssetDescription(parentAsset) : undefined;
